...
namespace VayneHunter2._0
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            var orbMenu = new Menu("Orbwalker", "Orbwalker1");
            ...
            Menu.SubMenu("Misc")
                .AddItem(
                    new MenuItem("ENextAuto", "Use E after next AA").SetValue(
            ...
            Menu.SubMenu("ManaMan").AddItem(new MenuItem("QManaC", "Min Q Mana in Combo").SetValue(new Slider(30, 1)));
            Menu.SubMenu("ManaMan").AddItem(new MenuItem("QManaM", "Min Q Mana in Mixed").SetValue(new Slider(30, 1)));
            Menu.SubMenu("ManaMan").AddItem(new MenuItem("EManaC", "Min E Mana in Combo").SetValue(new Slider(20, 1)));
            Menu.SubMenu("ManaMan").AddItem(new MenuItem("EManaM", "Min E Mana in Mixed").SetValue(new Slider(20, 1)));
            //Thank you blm95 ;)
            ...
            Orbwalking.AfterAttack += OwAfterAttack;
            ...
            foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy))
            {
                ...
                for (int i = 0; i < pushDist; i += (int)hero.BoundingRadius)
                {
                    ...
                    Vector3 loc3 = pred.UnitPosition.To2D()
                        ...
                        .To3D();
                    Utility.DrawCircle(
                        loc3, 100f, IsWall(loc3) ? System.Drawing.Color.Red : System.Drawing.Color.Aqua, 4);
                }
            }
        }
*/

        public static void OwAfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            if (!unit.IsMe)
            {
                return;
        ...
        public static void OnProcessSpell(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (IsEn(spellName) && sender.IsValidTarget(550f) && IsEn("AntiGP") &&
                Gapcloser.Any(str => str.Contains(args.SData.Name)) && args.Target.IsMe)
            {
                CastE((Obj_AI_Hero) sender, true);
            }
            //NonTargeted GP
            if (IsEn(spellName) && sender.IsValidTarget(550f) && IsEn("AntiGP") &&
                Notarget.Any(str => str.Contains(args.SData.Name)) && Player.Distance(args.End) <= 320f)
            {
                CastE((Obj_AI_Hero) sender, true);
            }
        }
        ...
        public static void CastQ(Obj_AI_Hero targ)
        {
            ...
            if (IsEn("SmartQ") && Player.Distance(targ) >= Orbwalking.GetRealAutoAttackRange(null))
            {
                if (IsMode("Combo") && GetManaPer() >= Menu.Item("QManaC").GetValue<Slider>().Value)
                {
                    ...
                    var canGapclose = Player.Distance(targ) <= Orbwalking.GetRealAutoAttackRange(null) + tumbleRange;
